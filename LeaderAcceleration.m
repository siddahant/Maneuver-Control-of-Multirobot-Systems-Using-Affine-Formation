function output=LeaderAcceleration(u)
global leaderNum dim nodenum simTime
...
if currentTime>=time_start_scaledown && currentTime<=time_start_scaledown+time_span_scaledown
    % agent 3
    a_leader_all(2,3)=-Accel_slow_fast(time_start_scaledown, time_span_scaledown, currentTime, 1);
    % agent 3
    a_leader_all(2,2)=Accel_slow_fast(time_start_scaledown, time_span_scaledown, currentTime, 1);
end
...
if currentTime>=time_start_scaleup && currentTime<=time_start_scaleup+time_span_scaleup
    % agent 3
    a_leader_all(2,3)=Accel_slow_fast(time_start_scaleup, time_span_scaleup, currentTime, 1);
    % agent 3
    a_leader_all(2,2)=-Accel_slow_fast(time_start_scaleup, time_span_scaleup, currentTime, 1);
end
...
if currentTime>=time_start_rotate2 && currentTime<=time_start_rotate2+time_span_rotate2
    % agent 2
    acce_mag=-Accel_slow_fast(time_start_rotate2, time_span_rotate2, currentTime, 4);
    acce_vec=[0 -1]';
    a_leader_all(:,2)=acce_mag*acce_vec;
    % agent 3
    acce_mag=-Accel_slow_fast(time_start_rotate2, time_span_rotate2, currentTime, 4);
...
if currentTime>=time_start_leftward && currentTime<=time_start_leftward+time_span_leftward
    % agent 1
    ax=-f_sin(time_start_leftward,time_span_leftward,currentTime)/pi;
    a_leader_all(:,1)=[ax,0]';
    % agent 2
...
if currentTime>=time_start_affineVertical && currentTime<=time_start_affineVertical+time_span_affineVertical
    % agent 3
    a_leader_all(1,3)=-Accel_slow_fast(time_start_affineVertical, time_span_affineVertical, currentTime, 1.5);
    %
    a_leader_all(2,3)=Accel_slow_fast(time_start_affineVertical, time_span_affineVertical, currentTime, 2);
    % agent 3
    a_leader_all(2,2)=-Accel_slow_fast(time_start_affineVertical, time_span_affineVertical, currentTime, 2);
end
...
if currentTime>=time_start_backtonormal && currentTime<=time_start_backtonormal+time_span_backtonormal
    % x-direction: agent 3
    a_leader_all(1,3)=Accel_slow_fast(time_start_backtonormal, time_span_backtonormal, currentTime, 1.5);
    % y-direction
    % agent 3
    a_leader_all(2,3)=-Accel_slow_fast(time_start_backtonormal, time_span_backtonormal, currentTime, 2);
    % agent 2
    a_leader_all(2,2)=Accel_slow_fast(time_start_backtonormal, time_span_backtonormal, currentTime, 2);
end
